 import { motion, AnimatePresence } from 'framer-motion';
    import { Calendar, Users, Award, Star, ChevronDown, ChevronUp,Briefcase, BarChart } from 'lucide-react';
                                 ...
                                 <ul className="space-y-2">
                                     {description.experience.map((item, index) => (
                                         <motion.li
                                             ...
                                             transition={{delay: index * 0.1}}
                                         >
                                             <Briefcase className="mr-3 flex-shrink-0 text-blue-400" size={20}/>
                                             <span>{item}</span>
                                         </motion.li>
                                     ))}
                                 </ul>
                                 ...
                                 <ul className="space-y-2">
                                     {description.kpis.map((item, index) => (
                                         <motion.li
                                             ...
                                             transition={{delay: index * 0.1}}
                                         >
                                             <BarChart className="mr-3 flex-shrink-0 text-green-400" size={20}/>
                                             <span>{item}</span>
                                         </motion.li>
                                     ))}
                                 </ul>
             ...
             <main className="relative z-10">
                 ...
                 >
                     <h2 className="text-4xl font-semibold mb-6 text-yellow-400 flex items-center ">
                         <Award className="mr-3 text-yellow-400" size={32}/> Event Introduction
                     </h2>
                     <motion.p
                     ...
                     >
                         'Legacy 2024' is the annual award ceremony organized by AIESEC in SLIIT to
                         recognize and celebrate the contributions and achievements of its members. It is the most prestigious
                         night of the AIESEC in SLIIT's calendar, acknowledging hard work, dedication, and impact in personal
                 ...
                 <section className="mb-16">
                     ...
                     <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                         {Object.entries(roleMapping).map(([shortName, {fullName, icon, count}], index) => (
                             <motion.div
                                 key={index}
                             ...
                             >
                                 ...
                                 <p className="font-semibold text-lg text-yellow-300">{shortName}</p>
                                 <p className="text-sm mt-2 text-white">{fullName}</p>
                                 <p className="text-sm mt-2 font-bold text-white">Positions: {count}</p>
                             </motion.div>
                         ))}
                     </div>
                 </section>
                 ...
                 <section className="mb-16">
                     <h2 className="text-4xl font-semibold mb-6 text-yellow-400">Job Descriptions</h2>
                 ...
                 <section className="mb-16">
                     <h2 className="text-4xl font-semibold mb-6 text-yellow-400">Contact Info</h2>
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                         {[
                             ...
                             <motion.div
                                 ...
                                 whileHover={{scale: 1.05, boxShadow: "0 0 20px rgba(255, 215, 0, 0.3)"}}
                             >
                                 <h3 className="text-2xl text-white font-semibold mb-3 ">{contact.name}</h3>
